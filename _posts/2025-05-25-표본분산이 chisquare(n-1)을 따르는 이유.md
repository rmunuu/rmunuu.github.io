...
정규모집단에서 $$n$$개를 추출한 표본에서 표본분산 $$ S^2 = \sum_{i=1}^n ({X_i - \bar{X})^2} $$ 일 때 $$ \frac{(n-1)S^2}{\sigma^2} \sim \chi^2 (n-1) $$ 라는 것을 배웠을 것이다. 하지만 이것이 왜 진짜로 성립하는지는 입문 수준에서는 잘 다루지 않는다. (보통 저 식을 전개를 해서, $$ \chi^2(n) - \chi^2(1) = \chi^2(n-1) $$ 이 된다고 설명한다. 이것도 $$\chi^2(n)$$과 $$\chi^2(1)$$이 독립일 때 성립하는 것인데, 입문 수준에서는 독립인 이유를 잘 설명하지 않는다.)
...
먼저 벡터와 행렬을 이용한 표본과 분포의 표현을 알고 가자. 오늘은 표본분산을 다루는데 집중할 것이므로 이 상황에 맞춰 설명하겠다. 정규모집단 $$ N(\mu, \sigma^2) $$ 에서 $$n$$개의 독립인 표본을 뽑았을 때, 우리는 $$ X_i \overset{\text{i.i.d.}}{\sim} N(\mu, \sigma^2) $$ 라고 썼다. $$ \mathbf{X} = [X_1 \; X_2 \;  ... \; X_n]^T $$, $$ \mathbf{1} = [1 \; 1 \; ... \; 1]^T $$ 일 때 이를 선형대수를 이용해 다시 쓰면 다음과 같이 표현할 수 있다.
